...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
function VoidParticles({ mouse, count }: { mouse: React.MutableRefObject<[number, number]>; count: number }) {
...
function SceneContent({ mouse, isMobile }: { mouse: React.MutableRefObject<[number, number]>; isMobile: boolean }) {
  ...
  // Use stable particle count regardless of mobile to avoid buffer resize errors
  const particleCount = 8000;

  return (
    <group ref={groupRef}>
      <VoidParticles key={`particles-${isMobile ? 'mobile' : 'desktop'}`} mouse={mouse} count={particleCount} />
      {sigils.map((s, i) => (
        <Sigil key={i} {...s} />
      ))}
